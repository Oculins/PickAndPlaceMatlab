...
% Copyright 2020 The MathWorks, Inc.
    ...
    [isAway, last_diffCu] = checkTargetAchieved();
    ...
    if isAway % if robot not at desired pose, compute a task-based joint trajectory
        ...
        count = 0;
        while isAway
            [isAway, diffCu] = checkTargetAchieved();
            if all(abs(last_diffCu - diffCu) < 0.05)
                count = count+1;
            end
            if count>10
                isAway = false;
    ...
    function [isAway, diffCurrent] = checkTargetAchieved()
        isAway = true;
        ...
        diffAngle = abs(angdiff(poseCurrent(4:6),poseTarget(4:6)));
        diffAngle = min(diffAngle, pi - diffAngle);
        diffCurrent = abs([poseTarget(1:3)-poseCurrent(1:3); diffAngle]);
